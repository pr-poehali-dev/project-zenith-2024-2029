...
export function DispatcherDashboard() {
  ...
  const handleParseStatistics = async () => {
    ...
    setStatusErr("")
    try {
      const { report, deviations, tasks: updatedTasks, matched } = await api.uploadStatistics(statsFile, selectedDate)
      setReport(report)
      setDeviations(deviations)
      setTasks(updatedTasks)
      setStatusMsg(`Отчёт сформирован: ${report.length} устройств обработано, данные внесены в ${matched} заданий`)
    } catch {
      setStatusErr("Ошибка при загрузке файла")
    } finally {
